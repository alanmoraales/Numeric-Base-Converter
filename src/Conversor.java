...
public class Conversor {
	...
	private List<Object> listaParteEntera = new ArrayList<Object>();
	private List<Object> listaParteFraccionaria = new ArrayList<Object>();
	...
	*/
	...
	public String toBaseN(float numeroDecimal, int baseObjetivo) {
		...
		//parte entera
		if(this.listaParteEntera.size() > 0) {
		...
		// parte fraccionaria
		if(this.listaParteFraccionaria.size() > 0) {
			// pasamos la parte fraccionaria a un string.
			auxString = "";
			for(int i=0; i<this.listaParteFraccionaria.size(); i++) {
				auxString = auxString + this.listaParteFraccionaria.get(i); // 0.xxx
			}
		...
		borrarLista(this.listaParteEntera);
		borrarLista(this.listaParteFraccionaria);
	...
	public float toDecimal(String numero, int base) {
		...
		if(this.listaParteEntera.size() > 0) {
			...
			for(int i=this.listaParteEntera.size() - 1; i>=0; i--) {
				
				if(Character.isDigit((char)this.listaParteEntera.get(i))) {
					auxInt = Character.getNumericValue((char) this.listaParteEntera.get(i)); // guardamos el dato en una variable entera.
					numeroDecimal = (float)(numeroDecimal + (auxInt * (Math.pow(base, potencia))));
				}
				// sino es numero, tenemos que encontrar el valor de la letra.
				else {
					auxChar = (char) this.listaParteEntera.get(i); // guardamos la letra en un auxiliar.
		...
		if(this.listaParteFraccionaria.size() > 0) {
			...
			for(int i=0; i<this.listaParteFraccionaria.size(); i++) {
				...
				if(Character.isDigit((char)this.listaParteFraccionaria.get(i))) {
					auxInt = Character.getNumericValue((char)this.listaParteFraccionaria.get(i)); // guardamos el dato en una variable entera.
					numeroDecimal = (float)(numeroDecimal + (auxInt * (Math.pow(base, -(i+1)))));
				}
				// sino es numero, tenemos que encontrar el valor de la letra.
				else {
					auxChar = (char) this.listaParteFraccionaria.get(i); // guardamos la letra en un auxiliar.
					auxInt = obtenerValor(auxChar); // obtenemos el valor.
	...
	private void ordenarParteEntera(String numero) {
		...
		*/
		...
		if(index != -1) { // indexOf devuelve -1 si no encuentra el caracter
			for(int i=0; i<index; i++) {
				this.listaParteEntera.add(arregloNumero[i]);
			}
		}
		else {
	...
	private void ordenarParteFraccionaria(String numero) {
		
		int index = numero.indexOf(".");
		char[] arregloNumero = numero.toCharArray();
		if(index != -1) { // indexOf devuelve -1 si no encuentra el caracter
			for(int i=index+1; i<arregloNumero.length; i++) {
				this.listaParteFraccionaria.add(arregloNumero[i]);
			}
		}
	...
	private String obtenerLetra(int valor) {
		String letra = "";
	...
	private int obtenerValor(char letra) {
	...
	private void borrarLista(List<Object> lista) {
